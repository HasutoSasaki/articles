...
解決策として、APIキー不要の検索パッケージ（`ddgs`。複数の検索エンジンをバックエンドに持つメタ検索ライブラリ）を導入する案も出ましたが、素性を十分に検証していない外部ライブラリを常時稼働のエージェントに追加することへの不安があり、この時点では導入を見送っています。原因は特定できたものの、web検索まわりはまだ未解決のままです。
...
- 解決策として提示されても、素性を検証できていない外部ライブラリは常時稼働のエージェントに安易に追加しない。web検索の問題のように、原因が分かっても対応を保留する判断も必要
